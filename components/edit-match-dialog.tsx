...
export function EditMatchDialog({ open, onOpenChange, onMatchUpdated, match, players }: EditMatchDialogProps) {
  const [player1Id, setPlayer1Id] = useState<string | null>(match.player1_id)
  const [player2Id, setPlayer2Id] = useState<string | null>(match.player2_id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error: editError } = await supabase.rpc("edit_match_simple", {
        p_match_id: match.id,
